...
//                . 'of this page is either invalid or has already been consumed.'
 ...
 //       }
         ...
         */
        $response = $this->exchangeCodeForUserId($code);
    ...
    $user = $_SERVER['HTTP_USER_AGENT'];
        if(strripos($user,'wxwork')){
            $queryParams = http_build_query($this->AuthorizeUrlParameters, '', '&', $this->AuthorizeUrlParametersEncType);
        }else{
            $this->authorizeUrl = 'https://open.work.weixin.qq.com/wwopen/sso/qrConnect';
            $queryParams = http_build_query($this->AuthorizeUrlParameters, '', '&', $this->AuthorizeUrlParametersEncType);
     ...
     * @throws \Hybridauth\Exception\HttpRequestFailedException
     */
    protected function exchangeCodeForUserId($code)
    {

        $this->tokenExchangeParameters['code'] = $code;
        $this->tokenExchangeParameters['access_token'] = $this->getStoredData('access_token');

//  throw new Exception(var_dump($this->tokenExchangeParameters));
    ...
    protected function validateAccessTokenExchange($response)
    {
        ...
        if (!$collection->exists('UserId')) {
            throw new InvalidAccessTokenException(
                'Provider returned no userid: ' . htmlentities($response)
            );
        }


        if ($collection->exists('errcode')) {
            $tmperr = $collection->get('errcode');
            if ($tmperr >0){
            throw new InvalidAccessTokenException(
                'Provider returned errcode: ' . htmlentities($response)
            );
            }
        }
    ...
    public function apiRequest($url, $method = 'GET', $parameters = [], $headers = [], $multipart = false)
    {
        ...
        $parameters = array("access_token"=>$this->getStoredData('access_token'),"userid"=>$this->getStoredData('userid'));
        $headers = array_replace($this->apiRequestHeaders, (array)$headers);
//  throw new Exception(var_dump($parameters));
    ...
    static $result=array();
        ...
        foreach($ids as $k => $v){
            $parameters = array("access_token"=>$this->getStoredData('access_token'),"id"=>$v);
            $headers = array_replace($this->apiRequestHeaders, (array)$headers);
            $response = $this->httpClient->request(
                $url,
                $method,     // HTTP Request Method. Defaults to GET.
        ...
        foreach($data->get('department') as $dpt){
            if ($dpt->{'id'} == $v){
                $result[] = $dpt->{'name'};
            if (isset($dpt->{'parentid'})){
              $aid = array($dpt->{'parentid'});
                  $this->getDepartment($aid);
           }       
            }
        }
    }
