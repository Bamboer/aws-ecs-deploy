...
func CreateTargetGroup(sess *session.Session, elb_name, target_group_name, target_type, protocol, protocolversion, vpcid string, target_group_port int64) *string {
	if sess == nil || target_group_name == "" {
	...
	// Create target group
	inputc := &elbv2.CreateTargetGroupInput{
		Name:            aws.String(target_group_name),
		Port:            aws.Int64(target_group_port),
		Protocol:        aws.String(protocol), //HTTP|HTTPS|TCP|TLS|UDP|GENEVE
		ProtocolVersion: aws.String(protocolversion),
		VpcId:           aws.String(vpcid),
		TargetType:      aws.String(target_type), //instance|ip|lambda
	}

	resultc, errc := svc.CreateTargetGroup(inputc)
	if err != nil {
	...
	Info.Println("Create target group: ", *resultc.TargetGroups[0].TargetGroupArn)
	return resultc.TargetGroups[0].TargetGroupArn
}
...
func CreateLB(sess *session.Session, lbname, elb_type, elb_scheme, security_group, subnets string) *string {
	...
	input := &elbv2.DescribeLoadBalancersInput{
		Names: aws.StringSlice(strings.Split(lbname, " ")),
	}
	...
	inputc := &elbv2.CreateLoadBalancerInput{
	...
	resultc, errc := svc.CreateLoadBalancer(inputc)
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			switch aerr.Code() {
	...
	Info.Println("Create ELB: ", resultc.LoadBalancers[0].LoadBalancerArn)
	return resultc.LoadBalancers[0].LoadBalancerArn
}

func CreateListener(sess *session.Session, lbname, protocol, elb_arn, target_group_arn string, port int64) *string {
	if sess == nil || target_group_arn == "" || lbname == "" {
		Error.Println("Not Create Listener.parameter not enough.")
		return nil
	}
	svc := elbv2.New(sess)
